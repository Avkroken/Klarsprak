...
Det required `validate`-jobbet kör regressionstesterna via `bun run test`, applicerar alla D1-migrationer mot en tom lokal databas och gör därefter en Wrangler dry-run. En regressionssvit som finns i repot men inte körs av `validate` ska betraktas som ofullständigt integrerad CI.
